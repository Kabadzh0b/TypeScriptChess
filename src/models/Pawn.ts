...
export class Pawn extends Figure {
  ...
  public canGoPush() {
    ...
    const x: number = this.square.x;
    const y: number = this.square.y;
    if (this.color === Colors.White) {
      if (x !== 0) {
        if (x === 6) {
          ...
          const sq: Square = this.square.board.getSquare(x - 1, y);
          if (sq.figure === null) {
            canGoArray.push(sq);
          }
        }
        if (y === 7) {
          const sq: Square = this.square.board.getSquare(x - 1, y - 1);
          if (sq.figure?.color === Colors.Black) canGoArray.push(sq);
        } else if (y === 0) {
          const sq = this.square.board.getSquare(x - 1, y + 1);
          if (sq.figure?.color === Colors.Black) canGoArray.push(sq);
        } else {
